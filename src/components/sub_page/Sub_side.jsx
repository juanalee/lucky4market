    ...
    const Sub_side = ({ isOpen, onClose, productImage, productInfo }) => {
      ...
      const purchaseDirect = useRef(null); // ref 추가
      const purchaseDelivery = useRef(null); // ref 추가
      ...
      useEffect(() => {
        if (isOpen) {
          tradeMethodArea.current.style.width = '600px';
          setTradeMethod(0);
        } else {
      ...
      const closePurchase = () => {
        ...
        resetTradeMethodSelection(); // 거래 방법 선택 초기화
        onClose();
      };
          ...
          <div className='purchase_side' ref={purchaseArea}>
            ...
            <div className='buyMethod_container'>
              ...
              <div className='buyMethod'>
                <span className={buyMethod === 'kakaopay' ? 'selected' : ''} onClick={() => setBuyMethod('kakaopay')}>카카오페이</span>
                <span className={buyMethod === 'tosspay' ? 'selected' : ''} onClick={() => setBuyMethod('tosspay')}>토스페이</span>
                <span className={buyMethod === 'payco' ? 'selected' : ''} onClick={() => setBuyMethod('payco')}>PAYCO</span>
                <span className={buyMethod === 'html5_inicis' ? 'selected' : ''} onClick={() => setBuyMethod('html5_inicis')}>카드결제</span>
              </div>
            </div>
            <hr />
